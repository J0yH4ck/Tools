...
def main():
    # 定义参数使用帮助
    usage = "python scanner.py -i <ip> -p <port> <Scanning method> 或者 python scanner.py -i <ip> -p all <Scanning method>"
    # 定义ArgumentParser 对象，用于解析命令行参数
    parser = argparse.ArgumentParser(description="Network scanning tool")
    
    # 添加命令行参数 -i 或者 --ipaddr，用于指定 IP 地址
    ...
    # python scanner.py -i <ip> -p 81,82 -sT
    ...
    # python scanner.py -i <ip> -p 81-85 -sT
    elif '-' in port and args.sT:
        #讲端口拆分为起始值和结束值
        port = port.split('-')
        s = int(port[0])
        ...
        for x in range(int(s), int(d)):
            ...
            t.start()
    # python scanner.py -i <ip> -p all -sT
